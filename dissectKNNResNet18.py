...
from topLatentFactorImageFunctions import topImagesPerLatentFactor, topMaskedImagesPerLatentFactor, topMaskedImagesPerLatentFactorAggPercent
...
def generateLatentImages(P,path,imgSize = 32):
	l = len(P)
	numImages = P[0].shape[1]
	for imgNum in range(numImages):
		image = []
		for channel in range(l):
			imgC = P[channel][:,imgNum].reshape(imgSize,imgSize)
			# pdb.set_trace()
...
if networkFile == "None":
	for epoch in range(0,numEpochs):
		...
		if epoch == numEpochs-1:
			...
			if actOnlyMode == 'False':
				...
				generateLatentImages(P,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,latentImages),imgSize = 32)
				os.chdir(os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging))
			...
			if classBased == 'True':
				...
				topMaskedImagesPerLatentFactorAggPercent(vF,iF,P,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','MaskedAggregatedandFractional0.1'),percent = 0.1)
				topMaskedImagesPerLatentFactorAggPercent(vF,iF,P,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','MaskedAggregatedandFractional0.2'),percent = 0.2)
				topMaskedImagesPerLatentFactorAggPercent(vF,iF,P,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','MaskedAggregatedandFractional0.3'), percent = 0.3)
				topMaskedImagesPerLatentFactorAggPercent(vF,iF,P,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','MaskedAggregatedandFractional0.4'),percent = 0.4)
				topMaskedImagesPerLatentFactorAggPercent(vF,iF,P,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','MaskedAggregatedandFractional0.5'), percent = 0.5)
			else:
				vF,iF,lF = analyzeF(F.T,CIFARval1.all_sampled_labels) # This needs to be fixed because we need index to class mapping,
				SvF,SiF,SlF = analyzeF(F.T,CIFARval1.all_sampled_super_labels)
	...
	if classBased == 'True':
		...
		SvF,SiF,SlF = analyzeF(F.T,CIFARval1.all_sampled_super_labels)
		topImagesPerLatentFactor(vF,iF,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','Raw'))
		topMaskedImagesPerLatentFactor(vF,iF,P,CIFARval1.all_sampled_image_paths,os.path.join(outputFolderName,epochFolder,analysisType,parentDirLatentImaging,topClassesLatentFactor,'topImagesPerLatentFactor','Masked'),imgSize = 32)
		knnLFDistMat, knnLFEnt = analyzeFKNN(F.T,targetList)
	...
	### Saving F Matrix ###
	np.save(os.path.join(os.getcwd(),'matrixF'),F)
